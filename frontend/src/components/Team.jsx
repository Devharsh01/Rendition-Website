    ...
    >
      <div className="absolute flex flex-col gap-[10px] pt-[10px] pointer-events-none">
        {imageChunks.map((imageChunk, rowIndex) => (
          <div
            className={`relative flex gap-[10px] w-max image-row ${
      ...
      <div className="relative ">
        ...
        >
          <div
            className={`title font-bold text-5xl md:text-7xl leading-[1.3em] `}
          >
            Team
          </div>
          <div className="relative flex justify-around items-center flex-wrap gap-2 w-full">
            {team.map((item, index) => (
              <div
                className={`relative text-white text-2xl md:text-4xl rounded-lg hover:bg-[#b20024] hover:text-white p-[10px] hover:cursor-pointer transition-all duration-500 ${
                  selectedBatch === item.batch
                    ? "bg-[#B20024] scale-110"
                    : "bg-[#2B1D4A] text-[#AFA1C9]"
                }`}
                onClick={() => {
            ...
            item.batch === selectedBatch && (
              <div className="relative flex flex-col md:flex-row w-full px-[5%] -mt-5 gap-5">
                        ...
                        {item.coordinators_photo.map((photo, photoIndex) => (
                          ...
                          >
                            <img
                              ...
                              className="rounded-full w-[4.5rem] md:w-[7.5rem] w-[4.5rem] md:h-[7.5rem] object-cover"
                            />
                            <p className="text-xl lg:text-2xl">
                              {item.coordinators[photoIndex]}
                            </p>
                          </motion.div>
                        ))}
                      </div>
                ...
                <div className="relative flex flex-col w-full h-full gap-2 items-center pb-2">
                  <h1
                    className={`mt-4 mb-4 sub-heading text-4xl lg:text-6xl text-transparent text-center font-bold bg-clip-text bg-gradient-to-t from-[#FF0033] to-[#FF4D70]`}
                  >
                    Team
                  </h1>
                  {imageLoader && (
                    <div className="relative text-2xl text-white top-32">
                      Loading...
                    </div>
                  )}
                  <img
