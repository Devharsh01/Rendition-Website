...
const Navbar = withLoadTracking(({ onLoad }) => {
  ...
  return (
    <div
      id="navbar"
      className={`fixed inset-0 flex justify-around items-center z-10 max-h-[150px] ${
        isScrolled ? "max-h-[60px] backdrop-blur-sm z-40" : ""
    ...
    >
      <div className="sm:hidden flex">
        ...
        <div className="flex flex-col z-20">
          ...
          <label
            htmlFor="checkbox"
            className={`relative w-10 h-10 cursor-pointer flex flex-col items-center justify-center gap-[10px] transition-all duration-300`}
          >
            {/* Bars for the checkbox */}
            <div
              className={`w-full h-[3px] bg-[#a8a8a8] rounded-[5px] transition-all duration-300 ${
                checked ? "ml-[13px] transform rotate-45 origin-left" : ""
              }`}
            />
            <div
              className={`w-full h-[3px] bg-[#a8a8a8] rounded-[5px] transition-all duration-300 ${
                checked ? "transform rotate-45 origin-center ml-0" : ""
              }`}
            />
            <div
              className={`w-full h-[3px] bg-[#a8a8a8] rounded-[5px] transition-all duration-300 ${
      ...
      {checked ? (
        <div className="fixed w-screen h-screen ">
          <div className="fixed w-screen px-6 overflow-hidden h-screen sm:hidden inset-0 bg-black bg-opacity-90 z-50 transition-opacity duration-300 ease-in-out">
            <div
              className={`relative ${
                isScrolled ? "mt-[90px]" : "mt-[120px]"
              } gap-[20px] flex flex-col justify-center items-center transform transition-transform duration-500 ease-in-out`}
            >
      ...
      {/* Rendition Logo */}
      <div
        className={`relative ml-[70px] sm:ml-0 h-[100px] lg:h-[150px] w-[100px] lg:h-[150px] -z-10 ${
      ...
      >
        <img
          src={logo}
          alt=""
          className="relative inset-0 h-full w-full object-contain"
        />
      </div>
      ...
      <div className="gap-[10px] md:gap-[30px] hidden sm:flex flex-row text-white">
        {pages.map((title, index) => (
          <p
            ...
            className={` text-content text-xl md:text-2xl lg:text-3xl cursor-pointer bg-gradient-to-t from-[#FF4D70] to-[#FF0033] bg-clip-text 
