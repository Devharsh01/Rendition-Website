...
const Productions = withLoadTracking(({ onLoad }) => {
  ...
  return (
    <div
      id="Productions"
      className="relative bg-gradient-to-br from-[#000000] via-[#1d002d] to-[#000000] w-full h-auto overflow-hidden flex flex-col p-4"
    >
      ...
      <div className="relative flex-grow w-full flex flex-col md:flex-row gap-5 px-4 md:px-8 pb-16">
        ...
        <div className="w-[95%] md:w-1/4 gap-4 text-center flex flex-row md:flex-col">
          {performance.map((heading) => (
            <motion.button
              key={heading}
              whileHover={{
                scale: startingPositions[heading] !== -1 ? 1.05 : 1,
              }}
              whileTap={{ scale: startingPositions[heading] !== -1 ? 0.95 : 1 }}
              className={`text-content relative text-lg sm:text-xl md:text-2xl lg:text-3xl text-white rounded-lg w-full h-max py-3 md:py-4 flex items-center justify-center transition-all duration-300 ease-in-out overflow-hidden
                ${
                  acts[currentIndex].category === heading
                    ? "bg-gradient-to-r from-[#80001A] to-[#FF0033] "
                    : "bg-[#2B1D4A]/70 hover:bg-[#2B1D4A]/90"
                }
                ${
                  startingPositions[heading] === -1
            ...
            >
              ...
              {acts[currentIndex].category === heading && (
                <motion.span
                  className="absolute bottom-0 left-0 h-1 bg-[#FF8099]"
                  initial={{ width: 0 }}
                  animate={{ width: "100%" }}
        ...
        <div className="relative flex-grow h-full md:w-3/4 overflow-hidden rounded-lg bg-transparent shadow-xl flex flex-col ">
          ...
          <motion.div
            ...
            className="py-5 relative z-20 w-full px-4"
          >
            <h1 className="text-content text-white font-bold text-xl md:text-3xl text-center leading-normal mb-4">
          ...
          <div className="relative flex flex-grow overflow-hidden">
            {acts.map((act, i) => (
              ...
              >
                <div
                  className="flex w-11/12 md:w-4/5 h-auto aspect-video rounded-lg overflow-hidden shadow-2xl shadow-red-900/20"
          ...
          >
            <div className="p-2 rounded-full bg-[#100030]/50 backdrop-blur-sm hover:bg-red-900/70 transition-colors">
                ...
                <button
                  ...
                  onClick={() =>
                    setCurrentIndex(
                      Math.min(
                        acts.length - 1,
                        visibleDotIndices[visibleDotIndices.length - 1] + 1
                      )
                    )
                  }
                >
